...
AddEventHandler('inventory:server:combineItem', function(item, fromItem, toItem)
	local src = source
	local ply = FTDCore.Functions.GetPlayer(src)
...
AddEventHandler('inventory:server:CraftItems', function(itemName, itemCosts, amount, toSlot, points)
	local src = source
	local Player = FTDCore.Functions.GetPlayer(src)
...
AddEventHandler('inventory:server:GiveItem', function(name, inventory, item, amount)
	local src = source
	local Player = FTDCore.Functions.GetPlayer(src)
	local OtherPlayer = FTDCore.Functions.GetPlayer(tonumber(name))
	...
	if amount ~= 0 then
		if Player.Functions.RemoveItem(item.name, amount,false, item.info) and OtherPlayer.Functions.AddItem(item.name, amount,false, item.info) then
			TriggerClientEvent('FTDCore:Notify', src, "You gave " ..amount.. ' '..item.label..' to '..Target)
			TriggerClientEvent('inventory:client:ItemBox',src, FTDCore.Shared.Items[item.name], "remove")
			TriggerClientEvent('FTDCore:Notify', name, "You got " ..amount.. ' ' ..item.label..' from '..Sender)
			TriggerClientEvent('inventory:client:ItemBox',name, FTDCore.Shared.Items[item.name], "add")
		else
			TriggerClientEvent('FTDCore:Notify', src, "Cant give item!", "error")
		end
	end
end)
	...
	if not ply.state.inv_busy then
		if name ~= nil and id ~= nil then
			...
			if name == "stash" then
				if Stashes[id] ~= nil then
					if Stashes[id].isOpen then
						local Target = FTDCore.Functions.GetPlayer(Stashes[id].isOpen)
						if Target ~= nil then
			...
			elseif name == "trunk" then
				if Trunks[id] ~= nil then
					if Trunks[id].isOpen then
						local Target = FTDCore.Functions.GetPlayer(Trunks[id].isOpen)
						if Target ~= nil then
				...
				if (Trunks[id] ~= nil and Trunks[id].isOpen) or (FTDCore.Shared.SplitStr(id, "PLZI")[2] ~= nil and Player.PlayerData.job.name ~= "police") then
					secondInv.name = "none-inv"
			...
			elseif name == "glovebox" then
				if Gloveboxes[id] ~= nil then
					if Gloveboxes[id].isOpen then
						local Target = FTDCore.Functions.GetPlayer(Gloveboxes[id].isOpen)
						if Target ~= nil then
			...
			elseif name == "crafting" then
				...
				secondInv.inventory = other.items
				secondInv.slots = #other.items
			elseif name == "crafting2" then
				secondInv.name = "crafting2"
				secondInv.label = other.label
				secondInv.maxweight = 900000
				secondInv.inventory = other.items
				secondInv.slots = #other.items
			...
			elseif name == "otherplayer" then
				local OtherPlayer = FTDCore.Functions.GetPlayer(tonumber(id))
				if OtherPlayer ~= nil then
					secondInv.name = "otherplayer-"..id
					secondInv.label = "Player-"..id
					secondInv.maxweight = FTDCore.Config.Player.MaxWeight
					secondInv.inventory = OtherPlayer.PlayerData.items
					if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
						secondInv.slots = FTDCore.Config.Player.MaxInvSlots
					else
						secondInv.slots = FTDCore.Config.Player.MaxInvSlots - 1
					end
					Citizen.Wait(250)
				end
			else
		...
		TriggerClientEvent('FTDCore:Notify', src, 'Not Accessible', 'error')
	end 	
end)
...
AddEventHandler('inventory:server:UseItemSlot', function(slot)
	...
	if itemData ~= nil then
		local itemInfo = FTDCore.Shared.Items[itemData.name]
		...
		elseif itemData.useable then
			TriggerClientEvent("FTDCore:Client:UseItem", src, itemData)
			TriggerClientEvent('inventory:client:ItemBox', src, itemInfo, "use")
		end
	end
end)
...
AddEventHandler('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if (fromInventory == "player" or fromInventory == "hotbar") and (FTDCore.Shared.SplitStr(toInventory, "-")[1] == "itemshop" or toInventory == "crafting") and (FTDCore.Shared.SplitStr(toInventory, "-")[1] == "itemshop" or toInventory == "crafting2") then
		return
	end

	if (fromInventory == "player" or fromInventory == "hotbar") and (FTDCore.Shared.SplitStr(toInventory, "-")[1] == "itemshop" or toInventory == "crafting2") then
		return
	end
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			elseif FTDCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				local playerId = tonumber(FTDCore.Shared.SplitStr(toInventory, "-")[2])
				local OtherPlayer = FTDCore.Functions.GetPlayer(playerId)
				...
				if toItemData ~= nil then
					--Player.PlayerData.items[fromSlot] = toItemData
					local itemInfo = FTDCore.Shared.Items[toItemData.name:lower()]
					local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						OtherPlayer.Functions.RemoveItem(itemInfo["name"], toAmount, fromSlot)
						Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("ftd-log:server:CreateLog", "robbing", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** with player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
					end
				else
					local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("ftd-log:server:CreateLog", "robbing", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** to player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
				end
				local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
				OtherPlayer.Functions.AddItem(itemInfo["name"], fromAmount, toSlot, fromItemData.info)
			elseif FTDCore.Shared.SplitStr(toInventory, "-")[1] == "trunk" then
				local plate = FTDCore.Shared.SplitStr(toInventory, "-")[2]
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("ftd-log:server:CreateLog", "trunk", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
					local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("ftd-log:server:CreateLog", "trunk", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
				end
				local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
				AddToTrunk(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif FTDCore.Shared.SplitStr(toInventory, "-")[1] == "glovebox" then
				local plate = FTDCore.Shared.SplitStr(toInventory, "-")[2]
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("ftd-log:server:CreateLog", "glovebox", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
					local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("ftd-log:server:CreateLog", "glovebox", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
				end
				local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
				AddToGlovebox(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif FTDCore.Shared.SplitStr(toInventory, "-")[1] == "stash" then
				local stashId = FTDCore.Shared.SplitStr(toInventory, "-")[2]
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("ftd-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
					end
				else
					local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("ftd-log:server:CreateLog", "stash", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
				end
				local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
				AddToStash(stashId, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif FTDCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				-- Traphouse
				local traphouseId = FTDCore.Shared.SplitStr(toInventory, "-")[2]
				local toItemData = exports['ftd-traphouse']:GetInventoryData(traphouseId, toSlot)
				local IsItemValid = exports['ftd-traphouse']:CanItemBeSaled(fromItemData.name:lower())
				if IsItemValid then
					Player.Functions.RemoveItem(fromItemData.name, fromAmount, fromSlot)
					TriggerClientEvent("inventory:client:CheckWeapon", src, fromItemData.name)
					if toItemData ~= nil then
						...
						local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
						if toItemData.name ~= fromItemData.name then
							exports['ftd-traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount)
							Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
							TriggerEvent("ftd-log:server:CreateLog", "traphouse", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
						end
					else
						local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
						TriggerEvent("ftd-log:server:CreateLog", "traphouse", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
					end
					local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
					exports['ftd-traphouse']:AddHouseItem(traphouseId, toSlot, itemInfo["name"], fromAmount, fromItemData.info, src)
				else
					TriggerClientEvent('FTDCore:Notify', src, "You can\'t sell this item..", 'error')
				end
			else
				-- drop
				...
				if toInventory == nil or toInventory == 0 then
					...
					if toItemData ~= nil then
						...
						if toItemData.name ~= fromItemData.name then
							...
							TriggerEvent("ftd-log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
						end
					else
						local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
						TriggerEvent("ftd-log:server:CreateLog", "drop", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
					end
					local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
			...
			TriggerClientEvent("FTDCore:Notify", src, "You don\'t have this item!", "error")
		end
	elseif FTDCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
		local playerId = tonumber(FTDCore.Shared.SplitStr(fromInventory, "-")[2])
		local OtherPlayer = FTDCore.Functions.GetPlayer(playerId)
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			TriggerClientEvent("FTDCore:Notify", src, "Item doesn\'t exist??", "error")
		end
	elseif FTDCore.Shared.SplitStr(fromInventory, "-")[1] == "trunk" then
		local plate = FTDCore.Shared.SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("ftd-log:server:CreateLog", "trunk", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("ftd-log:server:CreateLog", "trunk", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
					TriggerEvent("ftd-log:server:CreateLog", "trunk", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) reveived item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
				Player.Functions.AddItem(fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = Trunks[plate].items[toSlot]
	...
	elseif FTDCore.Shared.SplitStr(fromInventory, "-")[1] == "glovebox" then
		local plate = FTDCore.Shared.SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("ftd-log:server:CreateLog", "glovebox", "Swapped", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src..")* swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("ftd-log:server:CreateLog", "glovebox", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
					TriggerEvent("ftd-log:server:CreateLog", "glovebox", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) reveived item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
				Player.Functions.AddItem(fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = Gloveboxes[plate].items[toSlot]
	...
	elseif FTDCore.Shared.SplitStr(fromInventory, "-")[1] == "stash" then
		local stashId = FTDCore.Shared.SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("ftd-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. stashId .. "*")
					else
						TriggerEvent("ftd-log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. stashId .. "*")
					end
				else
					TriggerEvent("ftd-log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) reveived item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. stashId .. "*")
				end
	...
	elseif FTDCore.Shared.SplitStr(fromInventory, "-")[1] == "traphouse" then
		local traphouseId = FTDCore.Shared.SplitStr(fromInventory, "-")[2]
		local fromItemData = exports['ftd-traphouse']:GetInventoryData(traphouseId, fromSlot)
		local fromAmount = tonumber(fromAmount) ~= nil and tonumber(fromAmount) or fromItemData.amount
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
			if toInventory == "player" or toInventory == "hotbar" then
				local toItemData = Player.Functions.GetItemBySlot(toSlot)
				exports['ftd-traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], fromAmount)
				if toItemData ~= nil then
					local itemInfo = FTDCore.Shared.Items[toItemData.name:lower()]
					local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						Player.Functions.RemoveItem(toItemData.name, toAmount, toSlot)
						exports['ftd-traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
						TriggerEvent("ftd-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. traphouseId .. "*")
					else
						TriggerEvent("ftd-log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. traphouseId .. "*")
					end
				else
					TriggerEvent("ftd-log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) reveived item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. traphouseId .. "*")
				end
				Player.Functions.AddItem(fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = exports['ftd-traphouse']:GetInventoryData(traphouseId, toSlot)
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						local itemInfo = FTDCore.Shared.Items[toItemData.name:lower()]
						exports['ftd-traphouse']:RemoveHouseItem(traphouseId, toSlot, itemInfo["name"], toAmount)
						exports['ftd-traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
					end
				end
				local itemInfo = FTDCore.Shared.Items[fromItemData.name:lower()]
	...
	elseif FTDCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		local shopType = FTDCore.Shared.SplitStr(fromInventory, "-")[2]
		local itemData = ShopItems[shopType].items[fromSlot]
		local itemInfo = FTDCore.Shared.Items[itemData.name:lower()]
		local bankBalance = Player.PlayerData.money["bank"]
		local price = tonumber((itemData.price*fromAmount))

		if FTDCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if FTDCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					TriggerClientEvent('ftd-drugs:client:updateDealerItems', src, itemData, 1)
					TriggerClientEvent('FTDCore:Notify', src, itemInfo["label"] .. " bought!", "success")
					TriggerEvent("ftd-log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
				else
					TriggerClientEvent('FTDCore:Notify', src, "You don\'t have enough cash..", "error")
				end
			else
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					Player.Functions.AddItem(itemData.name, fromAmount, toSlot, itemData.info)
					TriggerClientEvent('ftd-drugs:client:updateDealerItems', src, itemData, fromAmount)
					TriggerClientEvent('FTDCore:Notify', src, itemInfo["label"] .. " bought!", "success")
					TriggerEvent("ftd-log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. "  for $"..price)
				else
					TriggerClientEvent('FTDCore:Notify', src, "You don't have enough cash..", "error")
		...
		elseif FTDCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
			if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
				if FTDCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
                    itemData.info.serie = tostring(Config.RandomInt(2) .. Config.RandomStr(3) .. Config.RandomInt(1) .. Config.RandomStr(2) .. Config.RandomInt(3) .. Config.RandomStr(4))
                end
				Player.Functions.AddItem(itemData.name, fromAmount, toSlot, itemData.info)
				TriggerClientEvent('ftd-shops:client:UpdateShop', src, FTDCore.Shared.SplitStr(shopType, "_")[2], itemData, fromAmount)
				TriggerClientEvent('FTDCore:Notify', src, itemInfo["label"] .. " bought!", "success")
				TriggerEvent("ftd-log:server:CreateLog", "shops", "Shop item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent('FTDCore:Notify', src, "You don't have the right items..", "error")
		end
	elseif fromInventory == "crafting2" then
		local itemData = Config.Craftingwps[fromSlot]
		if hasCraftItems(src, itemData.costs, fromAmount) then
			TriggerClientEvent("inventory:client:CraftItems", src, itemData.name, itemData.costs, fromAmount, toSlot, itemData.points)
		else
			TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("ftd-log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** - dropid: *" .. fromInventory .. "*")
					else
						TriggerEvent("ftd-log:server:CreateLog", "drop", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** - from dropid: *" .. fromInventory .. "*")
					end
				else
					TriggerEvent("ftd-log:server:CreateLog", "drop", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) reveived item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** -  dropid: *" .. fromInventory .. "*")
				end
				Player.Functions.AddItem(fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				toInventory = tonumber(toInventory)
...
function hasCraftItems(source, CostItems, amount)
	local Player = FTDCore.Functions.GetPlayer(source)
...
function SetupShopItems(shop, shopItems)
	...
	if shopItems ~= nil and next(shopItems) ~= nil then
		for k, item in pairs(shopItems) do
			local itemInfo = FTDCore.Shared.Items[item.name:lower()]
			items[item.slot] = {
				name = itemInfo["name"],
...
FTDCore.Functions.CreateCallback('ftd-inventory:server:GetStashItems', function(source, cb, stashId)
	cb(GetStashItems(stashId))
end)

RegisterServerEvent('ftd-inventory:server:SaveStashItems')
AddEventHandler('ftd-inventory:server:SaveStashItems', function(stashId, items)
...
function AddToStash(stashId, slot, otherslot, itemName, amount, info)
	local amount = tonumber(amount)
	local ItemData = FTDCore.Shared.Items[itemName]
	if not ItemData.unique then
		if Stashes[stashId].items[slot] ~= nil and Stashes[stashId].items[slot].name == itemName then
			...
			local itemInfo = FTDCore.Shared.Items[itemName:lower()]
...
function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
	...
	if Player.Functions.RemoveItem(itemData.name, itemAmount, itemData.slot) then
		...
		TriggerEvent("ftd-log:server:CreateLog", "drop", "New Item Drop", "red", "**".. GetPlayerName(source) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..source.."*) dropped new item; name: **"..itemData.name.."**, amount: **" .. itemAmount .. "**")
...
FTDCore.Commands.Add("resetinv", "Reset Inventory (Admin Only)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID of stash or license plate"}}, true, function(source, args)
	...
	if invType ~= nil and invId ~= nil then 
		...
		elseif invType == "stash" then
			...
			TriggerClientEvent('FTDCore:Notify', source,  "Not a valid type..", "error")
		end
	else
		TriggerClientEvent('FTDCore:Notify', source,  "Argumenten not filled out correctly..", "error")
	end
end, "admin")

FTDCore.Commands.Add("trunkpos", "View Trunk Location", {}, false, function(source, args)
	TriggerClientEvent("inventory:client:ShowTrunkPos", source)
end)

FTDCore.Commands.Add("rob", "Rob Player", {}, false, function(source, args)
	TriggerClientEvent("police:client:RobPlayer", source)
end)

FTDCore.Commands.Add("giveitem", "Give An Item (Admin Only)", {{name="id", help="Plaer ID"},{name="item", help="Name of the item (not a label)"}, {name="amount", help="Amount of items"}}, true, function(source, args)
	local Player = FTDCore.Functions.GetPlayer(tonumber(args[1]))
	local amount = tonumber(args[3])
	local itemData = FTDCore.Shared.Items[tostring(args[2]):lower()]
	if Player ~= nil then
		if amount > 0 then
			if itemData ~= nil then
				-- check iteminfo
				...
				elseif itemData["name"] == "labkey" then
					info.lab = exports["ftd-methlab"]:GenerateRandomLab()
				...
				if Player.Functions.AddItem(itemData["name"], amount, false, info) then
					TriggerClientEvent('FTDCore:Notify', source, "You Have Given " ..GetPlayerName(tonumber(args[1])).." "..amount.." "..itemData["name"].. "", "success")
				else
					TriggerClientEvent('FTDCore:Notify', source,  "Can't give item!", "error")
				end
			else
				TriggerClientEvent('FTDCore:Notify', source,  "Item Does Not Exist", "error")
			end
		else
			TriggerClientEvent('FTDCore:Notify', source,  "Invalid Amount", "error")
		end
	else
		TriggerClientEvent('FTDCore:Notify', source,  "Player Is Not Online", "error")
	end
end, "admin")

FTDCore.Commands.Add("randomitems", "Give Random Items (God Only)", {}, false, function(source, args)
	local Player = FTDCore.Functions.GetPlayer(source)
	local filteredItems = {}
	for k, v in pairs(FTDCore.Shared.Items) do
		if FTDCore.Shared.Items[k]["type"] ~= "weapon" then
	...
	for i = 1, 10, 1 do
		...
		if Player.Functions.AddItem(randitem["name"], amount) then
			TriggerClientEvent('inventory:client:ItemBox', source, FTDCore.Shared.Items[randitem["name"]], 'add')
            Citizen.Wait(500)
		end
	end
end, "god")

FTDCore.Functions.CreateUseableItem("snowball", function(source, item)
	local Player = FTDCore.Functions.GetPlayer(source)
	local itemData = Player.Functions.GetItemBySlot(item.slot)
...
FTDCore.Functions.CreateUseableItem("driver_license", function(source, item)
	for k, v in pairs(FTDCore.Functions.GetPlayers()) do
		local character = FTDCore.Functions.GetPlayer(source)
...
FTDCore.Functions.CreateUseableItem("id_card", function(source, item)
	for k, v in pairs(FTDCore.Functions.GetPlayers()) do
		local character = FTDCore.Functions.GetPlayer(source)
		local PlayerPed = GetPlayerPed(source)
		local TargetPed = GetPlayerPed(v)
