...
RegisterNetEvent('FTDCore:Client:OnPlayerLoaded')
AddEventHandler('FTDCore:Client:OnPlayerLoaded', function()
    LocalPlayer.state:set("inv_busy", false, true)
end)

RegisterNetEvent("FTDCore:Client:OnPlayerUnload")
AddEventHandler("FTDCore:Client:OnPlayerUnload", function()
...
AddEventHandler('inventory:client:CheckOpenState', function(type, id, label)
    local name = FTDCore.Shared.SplitStr(label, "-")[2]
    if type == "stash" then
...
Citizen.CreateThread(function()
    while true do
        ...
        if showTrunkPos and not inInventory then
            local vehicle = FTDCore.Functions.GetClosestVehicle()
            if vehicle ~= 0 and vehicle ~= nil then
                ...
                if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
                    ...
                    FTDCore.Functions.DrawText3D(drawpos.x, drawpos.y, drawpos.z, "Trunk")
...
RegisterCommand('inventory', function()
    if not isCrafting then
        FTDCore.Functions.GetPlayerData(function(PlayerData)
                    ...
                    if vehicle ~= 0 and vehicle ~= nil then
                        ...
                        if #(pos - trunkpos) < 2.0 and not IsPedInAnyVehicle(ped) then
                            if GetVehicleDoorLockStatus(vehicle) < 2 then
                                ...
                                FTDCore.Functions.Notify("Vehicle is locked..", "error")
                                return
                            end
                        else
                            CurrentVehicle = nil
    ...
    if bool then
        for k, v in pairs(items) do
            table.insert(itemTable, {
                item = items[k].name,
                label = FTDCore.Shared.Items[items[k].name]["label"],
                image = items[k].image,
            })
        end
    end
...
RegisterNUICallback('Notify', function(data, cb)
    FTDCore.Functions.Notify(data.message, data.type)
end)
...
AddEventHandler("inventory:client:OpenInventory", function(PlayerAmmo, inventory, other)
    if not IsEntityDead(PlayerPedId()) then
        ...
        SendNUIMessage({
            ...
            other = other,
            maxweight = FTDCore.Config.Player.MaxWeight,
            Ammo = PlayerAmmo,
...
function GetClosestPlayer()
    local closestPlayers = FTDCore.Functions.GetPlayersFromCoords()
        ...
        FTDCore.Functions.GetPlayerData(function(PlayerData)
            ...
            if player ~= -1 and distance < 2.5 then  --checks distance first
                ...
                if dist < 2.5 and not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then --checks what the player is doing
                    ...
                    FTDCore.Functions.Notify("Cant give item!", "error")
                end
            else
                FTDCore.Functions.Notify("No one nearby!", "error")
            end
        end)
    else
        FTDCore.Functions.Notify("Cant give item!", "error")
...
AddEventHandler("inventory:client:UpdatePlayerInventory", function(isError)
    SendNUIMessage({
        action = "update",
        inventory = FTDCore.Functions.GetPlayerData().items,
        maxweight = FTDCore.Config.Player.MaxWeight,
        slots = MaxInventorySlots,
        error = isError,
    })
...
AddEventHandler("inventory:client:CraftItems", function(itemName, itemCosts, amount, toSlot, points)
    ...
    isCrafting = true
    FTDCore.Functions.Progressbar("repair_vehicle", "Crafting..", (math.random(2000, 5000) * amount), false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', FTDCore.Shared.Items[itemName], 'add')
        isCrafting = false
	end, function() -- Cancel
		StopAnimTask(ped, "mini@repair", "fixing_a_player", 1.0)
        FTDCore.Functions.Notify("Failed!", "error")
        isCrafting = false
	end)
end)

RegisterNetEvent("inventory:client:craftweapons")
AddEventHandler("inventory:client:craftweapons", function(itemName, itemCosts, amount, toSlot, points)
    local ped = PlayerPedId()
    SendNUIMessage({
        action = "close",
    })
...
AddEventHandler("inventory:client:PickupSnowballs", function()
    ...
    FTDCore.Functions.Progressbar("pickupsnowball", "Sneeuwballen oprapen..", 1500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ClearPedTasks(ped)
        TriggerServerEvent('FTDCore:Server:AddItem', "snowball", 1)
        TriggerEvent('inventory:client:ItemBox', FTDCore.Shared.Items["snowball"], "add")
    end, function() -- Cancel
        ClearPedTasks(ped)
        FTDCore.Functions.Notify("Canceled..", "error")
    end)
end)
...
AddEventHandler("inventory:client:UseWeapon", function(weaponData, shootbool)
    ...
    elseif weaponName == "weapon_snowball" then
        ...
        TriggerServerEvent('FTDCore:Server:RemoveItem', weaponName, 1)
        TriggerEvent('weapons:client:SetCurrentWeapon', weaponData, shootbool)
        currentWeapon = weaponName
    else
        TriggerEvent('weapons:client:SetCurrentWeapon', weaponData, shootbool)
        FTDCore.Functions.TriggerCallback("weapon:server:GetWeaponAmmo", function(result)
...
RegisterNUICallback('GetWeaponData', function(data, cb)
    local data = {
        WeaponData = FTDCore.Shared.Items[data.weapon],
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    local ped = PlayerPedId()
    local WeaponData = FTDCore.Shared.Items[data.WeaponData.name]
    local Attachment = WeaponAttachments[WeaponData.name:upper()][data.AttachmentData.attachment]
    
    FTDCore.Functions.TriggerCallback('weapons:server:RemoveAttachment', function(NewAttachments)
...
RegisterNUICallback('getCombineItem', function(data, cb)
    cb(FTDCore.Shared.Items[data.item])
end)
...
RegisterNUICallback("combineItem", function(data)
    ...
    TriggerEvent('inventory:client:ItemBox', FTDCore.Shared.Items[data.reward], 'add')
end)
...
RegisterNUICallback('combineWithAnim', function(data)
    ...
    FTDCore.Functions.Progressbar("combine_anim", animText, animTimeout, false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', FTDCore.Shared.Items[combineData.reward], 'add')
    end, function() -- Cancel
        StopAnimTask(ped, aDict, aLib, 1.0)
        FTDCore.Functions.Notify("Failed!", "error")
    end)
...
function ToggleHotbar(toggle)
    local HotbarItems = {
        [1] = FTDCore.Functions.GetPlayerData().items[1],
        [2] = FTDCore.Functions.GetPlayerData().items[2],
        [3] = FTDCore.Functions.GetPlayerData().items[3],
        [4] = FTDCore.Functions.GetPlayerData().items[4],
        [5] = FTDCore.Functions.GetPlayerData().items[5],
        [41] = FTDCore.Functions.GetPlayerData().items[41],
    } 
